...
#define LOGA(condition, fmt, ...) \
    if(!(condition)) \
    { \
        __android_log_assert(condition, LOG_TAG, "(%s:%u) %s: error:%s " fmt, __FILE__, __LINE__, \
        __PRETTY_FUNCTION__, condition, ##__VA__ARGS__) \
    }
#else
#define LOGA(...) LOGN
#endif
#else
#include <stdio.h>
#define LOGP(fmt, ...) printf("%s line:%d" fmt, __FILE__, __LINE__, ##__VA__ARGS__)
#define LOGV(fmt, ...) LOGP(fmt, ##__VA__ARGS__)
#define LOGD(fmt, ...) LOGP(fmt, ##__VA__ARGS__)
#define LOGI(fmt, ...) LOGP(fmt, ##__VA__ARGS__)
#define LOGW(fmt, ...) LOGP(fmt, ##__VA__ARGS__)
#define LOGE(fmt, ...) LOGP(fmt, ##__VA__ARGS__)
#define LOGF(fmt, ...) LOGP(fmt, ##__VA__ARGS__)
#define LOGA(fmt, ...) LOGN
